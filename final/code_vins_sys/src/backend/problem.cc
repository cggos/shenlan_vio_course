...
Problem::Problem(ProblemType problemType) :
    ...
    file_time_.open("file_time_mt.txt", fstream::app | fstream::out);
}
...
//        if (sqrt(currentChi_) < 1e-15)
    ...
    file_time_ << iter << " " << t_hessian_cost_ << " " << t_solve.toc() << std::endl;
    t_hessian_cost_ = 0.;
    return true;
}
...
void Problem::MakeHessianOMP() {
    ...
    std::vector<unsigned long> edges_id;
    for(auto edge : edges_) {
        edges_id.push_back(edge.first);
    }

    // TODO:: accelate, accelate, accelate
    //for (auto &edge: edges_) {
    // openmp严格要求for循环下标必须是整数
    #pragma omp parallel for
    for(int idx=0; idx < edges_.size(); idx++ ) {
        //auto edge = *(edges_.begin()+idx); 
        // auto it = edges_.begin();
        // for(int i=0; i<idx; i++ ){
        //     ++it;
        // }
        auto edge = edges_[edges_id[idx]]; 
    ...
    if(H_prior_.rows() > 0)
    {
        ...
        for (auto vertex: verticies_) {
         // ----- new code start -----    
        // #pragma omp parallel for
        // for (int idx=0; idx<verticies_.size(); idx++ ){
        //     // 使用如下方法得到当前第 idx 个元素
        //     auto it = verticies_.begin();
        //     for(int i=0; i<idx; i++ ){
        //         ++it;
        //     }
        //     auto vertex = *it; 
...
void Problem::MakeHessianMT() {
    TicToc t_h;
    ulong size = ordering_generic_;
    mt_H_.setZero(size, size);
    mt_b_.setZero(size);

    for(auto edge : edges_) {
        mt_edges_id_.push_back(edge.first);
    }

    int n_thd = 2;
    int n_size = edges_.size();
    int len = 0;
    if(n_size%n_thd == 0)
        len = n_size / n_thd;
    else
    ...
    for(int i=0; i<n_thd; ++i) {
        int idx_start = i * len;
        if(i == n_thd-1)
            len = n_size - idx_start;
        int end = idx_start + len - 1;
        std::thread t = std::thread(&Problem::ThreadDoEdges, this, idx_start, end);
        t.join();
        // ThreadDoEdges(idx_start, end);
    }
    // for(int i=0; i<n_thd; ++i)
    //    t[i].join();
...
void Problem::ThreadDoEdges(int start, int end) {
    for(int n=start; n<=end; n++ ) {
        auto edge = edges_[mt_edges_id_[n]]; 
        ...
        for (size_t i = 0; i < verticies.size(); ++i) {
            ...
            for (size_t j = i; j < verticies.size(); ++j) {
                ...
                // 所有的信息矩阵叠加起来
                mtx_h_.lock();
                mt_H_.block(index_i, index_j, dim_i, dim_j).noalias() += hessian;
                if (j != i) {
                    // 对称的下三角
                    mt_H_.block(index_j, index_i, dim_j, dim_i).noalias() += hessian.transpose();
                }
                mtx_h_.unlock();
            }
            mtx_b_.lock();
            mt_b_.segment(index_i, dim_i).noalias() -= drho * jacobian_i.transpose()* edge->Information() * edge->Residual();
            mtx_b_.unlock(); 
        }
    }
}
...
bool Problem::IsGoodStepInLM() {
    IsGoodStepInLMNormal();
}

bool Problem::IsGoodStepInLMNormal() {
    double scale = 0;
